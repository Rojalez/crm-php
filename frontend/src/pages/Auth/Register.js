        ...
        <div className="flex flex-col items-center justify-center">     
            <form className="bg-grey-800 shadow flex flex-col justify-center items-center rounded lg:w-1/3 md:w-1/2 w-full p-10 mt-16" onSubmit={handleSubmit}>
                ...
                <div className='mt-6 w-full'>
                    ...
                    <input required type="text" onChange={e => setName(e.target.value)} className="bg-grey-700  placeholder-grey-300 text-white border-0 rounded focus:outline-none text-xs font-medium leading-none  py-3 px-2 w-full mt-2" />
                </div>
                <div className='mt-6 w-full'>
                    <span className="text-sm font-medium leading-none text-white">Почта</span>
                    <input required type="email" autoComplete='username' onChange={e => setEmail(e.target.value)} className="bg-grey-700  placeholder-grey-300 text-white border-0 rounded focus:outline-none text-xs font-medium leading-none  py-3 px-2 w-full mt-2" />
                </div>
                <div className="mt-6 w-full">
                    <span className="text-sm font-medium leading-none text-white">Пароль</span>
                    <input required type="password" autoComplete="current-password" onChange={e => setPassword(e.target.value)} className="bg-grey-700  placeholder-grey-300 text-white border-0 rounded focus:outline-none text-xs font-medium leading-none  py-3 px-2 w-full mt-2" />
                </div>
